...
def save_lvc_metrics_to_wsf_format(report_dir, summary_file, live_caption_duration_seconds):
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        for run_id, metrics in summary.items():
            ...
            writer.writerow([f"Resolution Width", metrics.get("Resolution_Width", 0)])
            writer.writerow([f"Frame Rate", metrics.get("Frame Rate", 0)])
            writer.writerow([f"Chunk Size", metrics.get("Chunk Size", 0)])
            writer.writerow([])
