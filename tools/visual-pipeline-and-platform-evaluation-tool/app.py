...
def find_shm_file():
    """
    Find the actual shared memory file in /dev/shm that matches the shmpipe.* pattern.
    Returns the filename or None.
    """
    logger = logging.getLogger("find_shm_file")
    try:
...
def read_latest_meta(meta_path):
    ...
    """
    logger = logging.getLogger("read_latest_meta")
    try:
...
def read_shared_memory_frame(meta_path="/tmp/shared_memory/video_stream.meta", shm_prefix="shmpipe.video_stream", socket_path="/tmp/shared_memory/video_stream"):
    ...
    """
    logger = logging.getLogger("read_shared_memory_frame")
...
def live_preview_stream(meta_path="/tmp/shared_memory/video_stream.meta", shm_prefix="shmpipe.video_stream", stop_event=None, socket_path="/tmp/shared_memory/video_stream"):
    ...
    logger = logging.getLogger("live_preview_stream")
    frame_count = 0
...
def run_pipeline_and_stream(data):
    ...
    logger.debug(
        f"Detection Model: {arguments.get('object_detection_model')}, Device: {arguments.get('object_detection_device')}")
    logger.debug(
        f"Classification Model: {arguments.get('object_classification_model')}, Device: {arguments.get('object_classification_device')}")
    ...
    # Wait briefly to allow client to finish reading shared memory
    time.sleep(1)

    # Clean up shared memory file(s)
    try:
        shm_files = [f for f in os.listdir("/dev/shm") if f.startswith("shmpipe.")]
        for f in shm_files:
            os.remove(os.path.join("/dev/shm", f))
    except Exception as e:
        logger.warning(f"Could not clean up shared memory files: {e}")
...
def create_interface(title: str = "Visual Pipeline and Platform Evaluation Tool"):
    ...
    with gr.Blocks(theme=theme, css=css_code, title=title) as demo:
        ...
        run_button.click(
            ...
            inputs=components,
            outputs=[output_live_image, output_video_player],  # live first, then file
        ).then(
