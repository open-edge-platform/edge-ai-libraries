...
6. **Access the Application**:
    ...
    - **Expected Results**:
      ...
      - The Smart NVR pipeline is automatically executed when the "Run" button is clicked, and the output video is shown
        with device metrics.
...
1. **Change GPU Selection Between Available Integrated and Discrete GPU**:
    ...
    - **Update the compose.yaml File**:
      Modify the `compose.yaml` file to specify the discrete GPU device by updating the `/dev/dri` path to the
      appropriate `renderXXXX` device as "/dev/dri/renderXXXX". Note that usually the discrete GPU is mentioned as
      "renderD129".
