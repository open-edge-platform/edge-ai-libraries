...
class Graph:
    ...
    def apply_looping_modifications(self) -> "Graph":
        ...
        - Change file extension from .mp4 to .ts in location
        ...
        for node in modified_graph.nodes:
            ...
            if node.type == "filesrc":
                ...
                # TODO: Extension handling could be improved to support other formats (not only .mp4) and can be done in VideosManager class
                # TODO: VideoManager should also convert input video to .ts format after downloading (if downloaded video is not *ts)
                # TODO: Example of conversion command for *.mp4 to *.ts (which can be run by PipelineRunner after downloading the video):
                ## gst-launch-1.0 filesrc location=/videos/input/people.mp4 ! qtdemux name=mdemux ! h264parse ! video/x-h264,stream-format=byte-stream ! mpegtsmux name=mux ! filesink location=/videos/input/people.ts
                # TODO: Each *.ts file should also have corresponding *.json metadata file generated by VideosManager class
                # Change file extension from .mp4 to .ts
                if "location" in node.data:
                    location = node.data["location"]
                    if location.endswith(".mp4"):
                        node.data["location"] = location[:-4] + ".ts"
                    logger.debug(
                        f"Modified filesrc to multifilesrc with location: {node.data['location']}"
                    )

            # Replace qtdemux with tsdemux
            elif node.type == "qtdemux":
                node.type = "tsdemux"
                logger.debug("Replaced qtdemux with tsdemux for looping support")
