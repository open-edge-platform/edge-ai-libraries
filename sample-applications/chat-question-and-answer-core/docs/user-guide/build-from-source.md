...
2. Build the Docker image using the provided `Dockerfile` based on your setup and prefered model framework.
   ...
   - **OpenVINO™ toolkit framework**
     ...
     - CPU-only inferencing (Default Configuration):
       ...
       ```bash
       docker build -t chatqna:core_2026.2.0 -f docker/Dockerfile .
       ```
     ...
     - GPU-enabled inferencing (Intel GPUs support):
       ...
       ```bash
       docker build -t chatqna:core_gpu_2026.2.0 --build-arg USE_GPU=true -f docker/Dockerfile .
       ```
   ...
   - **Ollama framework**
     ...
     - CPU-only inferencing
        ...
        ```bash
        docker build -t chatqna:core_ollama_2026.2.0 -f docker/Dockerfile.ollama .
        ```
...
2. Build the Docker image using the provided `Dockerfile`:
   ...
   ```bash
   docker build -t chatqna-ui:core_2026.2.0 .
   ```
