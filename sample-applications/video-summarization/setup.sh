...
convert_object_detection_models() {
    ...
    python3 ../../microservices/video-ingestion/resources/scripts/converter.py --model-name "${OD_MODEL_NAME}" --model-type "${OD_MODEL_TYPE}" --output-dir "${OD_MODEL_OUTPUT_DIR}"
...
if [ "$1" = "--summary" ] || [ "$1" = "--all" ]; then
    ...
    if [ "$ENABLE_OVMS_LLM_SUMMARY" = true ] || [ "$ENABLE_OVMS_LLM_SUMMARY_GPU" = true ]; then
        ...
        export ENABLE_OVMS_LLM_SUMMARY_CONFIG=CONFIG_ON
        ...
        export ENABLE_OVMS_LLM_SUMMARY_CONFIG=CONFIG_OFF
