...
> **Note:** On resource-constrained worker nodes, prefer a smaller VLM such as `Qwen/Qwen2.5-VL-3B-Instruct` instead of `Qwen/Qwen2.5-VL-7B-Instruct`. For CPU deployments, you can also lower the VLM microservice worker count from the default `6` to `1` by editing the `WORKERS` setting in [vlm-inference-deployment.yaml](../../chart/subchart/vlm-inference/templates/vlm-inference-deployment.yaml#L64).
