...
class ProxyFilterConfig(BaseModel):
    ...
    resource_scheme: str = Field(
        default="api",
        description="URI scheme used for generated MCP resources (reserved; FastMCP manages URIs).",
    )
    ...
    def _validate_tool_name_uniqueness(self) -> "ProxyFilterConfig":
        ...
        for api_key, config in self.apis.items():
            if config.tool_name is None:
                continue
            previous = seen.get(config.tool_name)
...
def operation_key(method: str, path: str) -> str:
    """Return the normalized JSON key for an (method, path) pair."""
...
def api_config_for(
    ...
    """Return the explicit per-operation config for the given (method, path) pair."""
...
def resource_is_enabled(config: ProxyFilterConfig, method: str, path: str) -> bool:
    ...
    if api_config is None or api_config.expose != "resource":
        return False
    # Resources must be read-only requestless GETs.
