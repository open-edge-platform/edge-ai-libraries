...
│   └── filters/
│       └── config.py            # ProxyFilterConfig, ApiConfig, routing helpers
│
└── tests/
...
4. **Name mapping** — tool names are resolved to `{prefix}_{tool_name}`.
...
| `prefix`      | Prefix applied to every `tool_name` and `resource_name` (e.g. `"vss"` → `"vss_run_search_query"`) |
