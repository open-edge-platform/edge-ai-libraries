...
struct BranchStats {
    ...
    void cal_log_pipeline_latency(guint64 ts, guint64 init_ts, gint interval) {
        ...
        gst_tracer_record_log(tr_pipeline, source_name.c_str(), sink_name.c_str(), frame_latency, avg, min, max, pipeline_latency, fps, frame_count);
    ...
    void cal_log_pipeline_interval(guint64 ts, gdouble frame_latency, gint interval) {
        ...
        if (ms >= interval) {
            ...
            gst_tracer_record_log(tr_pipeline_interval, source_name.c_str(), sink_name.c_str(), ms, interval_avg, interval_min, interval_max, pipeline_latency,
                                  fps);
            reset_interval(ts);
        }
    }
...
static void cal_log_pipeline_interval(LatencyTracer *lt, guint64 ts, gdouble frame_latency, const char *source_name, const char *sink_name) {
    ...
    if (ms >= lt->interval) {
        ...
        gst_tracer_record_log(tr_pipeline_interval, source_name, sink_name, ms, interval_avg, lt->interval_min, lt->interval_max,
...
static void cal_log_pipeline_latency(LatencyTracer *lt, guint64 ts, LatencyTracerMeta *meta) {
    ...
    gst_tracer_record_log(tr_pipeline, source_name, sink_name, frame_latency, avg, lt->min, lt->max, pipeline_latency, fps, lt->frame_count);
