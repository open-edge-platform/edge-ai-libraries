...
class ZxingCpp(BarcodeDecoder):
    ...
    def decode(self, region_data):
        ...
        if barcode is None:
            return []
        (x, y, w, h) = (barcode.position.bottom_left.x,
                        barcode.position.bottom_left.y, barcode.position.top_right.x - barcode.position.bottom_left.x, barcode.position.bottom_left.y - barcode.position.top_right.y)
        new_label = "barcode: {}".format(
...
class BarcodeDetection:
    ...
    def __init__(self, disable=False, decode_type="zxingcpp", reclassify_interval=-1, max_tracked_objects=20):
