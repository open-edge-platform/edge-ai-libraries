...
<a name="setting-text-embedding-model"></a>
3. _(OPTIONAL)_ The microservice supports multimodal embedding models by default _(will use value of `VCLIP_MODEL` set in step 2, as embedding model, which can generate both text as well as image embeddings)_. But, if you want to exclusively generate text embeddings only, set the following environment variable:
...
> __NOTE:__ Only `openai/clip-vit-base-patch32` is supported right now, as multimodal embedding model. Similarly, as a text embedding model, only `Qwen/Qwen3-Embedding-0.6B` is supported right now. This should be apparent by looking the variables `VCLIP_MODEL` and `QWEN_MODEL`.
...
> _**NOTE:**_ Text embeddings can be generated by both text embedding models and multimodal embedding models. [Refer to this](#setting-text-embedding-model) to understand how to set models used for generating text embeddings.
