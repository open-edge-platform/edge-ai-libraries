...
class MobileCLIPHandler(BaseEmbeddingModel):
    ...
    def _load_openvino_models(self) -> None:
        ...
        # Load just for preprocessing (without full model)
        if self.url:
            model_dir = Path("checkpoints")
            model_path = model_dir / Path(self.url).name
            if model_path.exists():
                ...
                raise FileNotFoundError(f"MobileCLIP checkpoint not found at {model_path}")
        else:
            _, _, self.preprocess = mobileclip.create_model_and_transforms(
    ...
    def convert_to_openvino(self, ov_models_dir: str) -> tuple:
        """Convert MobileCLIP model to OpenVINO format."""
        ...
        if not image_encoder_path.exists():
            ...
            if "mobileclip_s" in self.model_name:
                self.model.image_encoder.model.conv_exp.se.forward = types.MethodType(
                    se_block_forward, 
                    self.model.image_encoder.model.conv_exp.se
                )
        ...
        if not text_encoder_path.exists():
            ...
            original_forward = self.model.forward
            self.model.forward = self.model.encode_text
