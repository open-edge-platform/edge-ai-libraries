...
from video_analyzer.utils.file_utils import validate_local_file_path, validate_remote_url
...
def load_subtitles(input, max_bytes: int = 10 * 1024 * 1024) -> List[Dict]:
    ...
    if isinstance(input, dict):
        ...
        if "url" in input and input["url"]:
            ...
            with requests.get(  # lgtm [py/full-ssrf]
                url,
                stream=True,
                timeout=30,
    ...
    if isinstance(input, str):
        ...
        path = validate_local_file_path(input)  # lgtm [py/path-injection]
