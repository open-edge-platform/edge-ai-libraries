...
If your Linux iGPU setup provides an Intel oneAPI environment script, it
must be sourced before starting the service:

```bash
source /opt/intel/oneapi/setvars.sh
```

This script is not present on a default Ubuntu install; it appears only
after the relevant Intel host stack is installed.
