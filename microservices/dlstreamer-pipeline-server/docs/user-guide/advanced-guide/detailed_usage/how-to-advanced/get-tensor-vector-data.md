...
6. You can check the vector output by subscribing to MQTT. You can check this [document](../../detailed_usage/publisher/mqtt_publish.md#start-mqtt-subscriber) on how to configure and start the MQTT subscriber.

   Here is what a sample metadata for a frame looks like (some data deleted to keep size small):
...
7. To stop DL Streamer Pipeline Server and other services, run:
