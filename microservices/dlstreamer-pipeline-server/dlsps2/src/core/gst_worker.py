...
Each hosted pipeline runs on its own private ``GLib.MainContext``/
``GLib.MainLoop`` pair on a dedicated thread, so one pipeline's bus activity
...
class _Instance:
    ...
    # from the MainLoop thread (inside the poll timer), hence the lock.
    # frame_count resets every poll (drives the windowed "last_fps");
...
class GstWorker:
    ...
    def _attach_fps_probes(pipeline: Gst.Pipeline, inst: "_Instance") -> int:
        """Attach a buffer-counting probe to every sink element's sink pad(s).

        Using ``iterate_sinks()`` (elements with no linked src pad, i.e. the
        actual terminal element(s) of the pipeline) means exactly one frame
        is counted per buffer that reaches the end of *this* pipeline,
        regardless of what's upstream. Returns the number of probes attached.
        """
        attached = 0
    ...
    def start(self, instance_id: str, pipeline_description: str) -> None:
        ...
        inst = _Instance(instance_id=instance_id, pipeline=pipeline, loop=loop, context=context)
        ...
        thread = threading.Thread(
            ...
            args=(inst,),
            name=f"worker-{instance_id[:8]}",
            daemon=True,
        )
        inst.thread = thread
        thread.start()

    def _run_instance(self, inst: _Instance) -> None:
        """Thread body: run one hosted pipeline on its own main context."""
        pipeline = inst.pipeline
        loop = inst.loop
        context = inst.context

        # Bus signal watches attach to the thread-default main context at
        # the moment add_signal_watch() is called, so push this instance's
        # private context as thread-default *before* wiring up the watch.
        ...
        try:
            ...
            def _on_message(_bus, message):
                ...
                if mtype == Gst.MessageType.ERROR:
                    ...
                    _emit({"instance_id": inst.instance_id, "event": "error", "reason": err.message})
                    loop.quit()
                elif mtype == Gst.MessageType.EOS:
                    _emit({"instance_id": inst.instance_id, "event": "eos"})
                    loop.quit()
                return True

            handler_id = bus.connect("message", _on_message)

            # Count frames ourselves via pad probes on this pipeline's own
            # sink element(s), then turn that into avg/last FPS values on a
            # timer attached to this pipeline's private context. This is
            # fully isolated per instance — no shared/native aggregation.
            ...
            fps_source.attach(context)

            pipeline.set_state(Gst.State.PLAYING)
            _emit({"instance_id": inst.instance_id, "event": "started"})
        ...
        del pipeline
        gc.collect()
        _emit({"instance_id": inst.instance_id, "event": "stopped"})
    ...
    def shutdown(self) -> None:
        """Stop every hosted pipeline and wait for their threads to finish."""
        with self._lock:
