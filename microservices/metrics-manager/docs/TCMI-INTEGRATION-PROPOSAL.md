...
| Turbostat R-dimension diagnostics — **opt-in, ships disabled** | [`telegraf.d/60-turbostat.conf.example`](../telegraf.d/60-turbostat.conf.example) (native `inputs.turbostat`) | no data until enabled (kernel-coupled binary; opt-in) |
...
| **P2 — Residual execd readers** ✅ **DONE (psys)** | `psys_power` (`rapl_reader.py`, live: 5.9 W psys at idle) landed + wired in `90-tcmi-execd.conf`. R-dimension is handled by the native `inputs.turbostat` plugin (Telegraf 1.36+; we build 1.38.4), opt-in via `60-turbostat.conf.example` — no custom reader needed. Still TODO: xe GPU throttle/freq, TCPU zones (if `inputs.temp` misses them), `npu_power_state` one-liner. | Medium | 1 PR: `scripts/*.py` + `90-tcmi-execd.conf` |
