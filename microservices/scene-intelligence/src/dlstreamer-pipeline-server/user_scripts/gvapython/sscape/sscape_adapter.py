    ...
    if self.ntpServer:
      ...
      if not self.lastTimeSync or now - self.lastTimeSync > 1000 :
        try:
          ...
          self.log.warning(f"NTP sync failed for {self.ntpServer}: {e}")
          # Continue without time sync if NTP fails
...
class PostInferenceDataPublish:
  ...
  def setupMQTT(self):
    ...
    # Set up TLS before connecting
    if ROOT_CA and os.path.exists(ROOT_CA):
      print(f"Setting up TLS with ROOT_CA: {ROOT_CA}")
      self.client.tls_set(ca_certs=ROOT_CA)
    else:
      print(f"Warning: ROOT_CA not found at {ROOT_CA}, connecting without TLS")
