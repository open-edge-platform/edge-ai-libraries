...
1. **Frame selection:** Samples up to `num_frames` key frames identified during pose matching; this value is configurable per pattern.
...
- **Logic:** When a new `ba/request` arrives for person ID `P`:
  - If `P` is in `_inflight_entities`: the request is silently dropped (logged at debug level)
  - If `P` is not in `_inflight_entities`: `P` is added, the analysis task is dispatched, and `P` is removed when the task completes
