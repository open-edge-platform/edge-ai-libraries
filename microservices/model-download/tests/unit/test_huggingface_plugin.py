...
class TestHuggingFaceListModels:
    ...
    @patch('src.plugins.huggingface_plugin.HfApi')
    def test_list_models_stringifies_search(self, mock_hfapi, hf_plugin):
        api = mock_hfapi.return_value
        api.list_models.return_value = iter([])

        hf_plugin.list_models(filters={"search": 123})

        assert api.list_models.call_args.kwargs["search"] == "123"
