...
GETI_LISTING_FILTER_FIELDS = [
    "project_id", "project_name", "model_name", "precision",
...
class GetiPlugin(ModelDownloadPlugin):
        ...
        for project in projects:
            ...
            for model in await self._get_project_models(session, project["id"]):
                ...
                if variant_id:
                    variants = [
                        variant for variant in variants
                        if str(variant.get("id", variant.get("variant_id"))) == variant_id
                    ]
                if model_format is not None or precision is not None:
                    variants, _ = self._filter_variants(variants, model_format, precision)
                # If any variant filter was given, drop models with no surviving variant.
                if (variant_id or model_format is not None or precision is not None) and not variants:
                    continue
    ...
    async def search_model(self, session: _GetiSession, model_name: str,
        ...
        for project in await self.get_projects(session):
            for model in await self._get_project_models(session, project["id"]):
                ...
                if not variants:
                    continue
                variant_id = variants[0].get("id", variants[0].get("variant_id"))
    ...
    async def download(self, model_name: str, output_dir: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            supported = {"precision", "model_format", "variant_id", "model_id", "project_id"}
            ...
            model_id, project_id, variant_id, error, ignored = await self.search_model(
                session, model_name, precision, model_format=model_format, extra_filters=extra_filters
