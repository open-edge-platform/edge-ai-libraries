...
1. Pull the specific chart
   ...
   ```bash
   helm pull oci://registry-1.docker.io/intel/model-download-chart --version 1.1.0
   ```
...
2. Extract the `.tgz` file
   ...
   ```bash
   tar -xvf model-download-chart-1.1.0.tgz
   ```
...
> **Note:** `model-download` creates and manages a shared PVC that can be used by dependent applications such as ChatQ&A.
