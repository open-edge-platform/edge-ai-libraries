...
- Embedding Backend:
        ...
        Generates CLIP embeddings either in-process via the bundled `multimodal_embedding_serving` SDK (default; raw `PIL.Image` frames are batched in a single `encode_image` call) or over HTTP against a remote `multimodal-embedding-serving` container. Selectable at runtime via `USE_SDK_EMBEDDING`.
