    ...
    try:
        ...
        for i, frame_idx in enumerate(frame_indices):
            try:
                ...
                logger.debug(
                    f"Processing frame {i}: frame_idx={frame_idx}, tensor_shape={frame_tensor.shape}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to convert frame tensor for frame {frame_idx}: {tensor_error}"
                )
                logger.error(
                    f"Frame tensor type: {type(frame_tensor)}, shape: {getattr(frame_tensor, 'shape', 'unknown')}"
                )
                continue
        ...
        if enable_object_detection and detector is not None:
            ...
            for batch_start in range(0, len(extracted_frames), detection_batch_size):
                ...
                logger.debug(
                    f"Processing detection batch {batch_start//detection_batch_size + 1}: frames {batch_start}-{batch_end-1}"
                )
                ...
                try:
                    ...
                    for frame_pil, meta in zip(batch_frames, batch_metadata):
                        ...
                        try:
                            ...
                            if len(crops) > 0:
                                for j, (crop, det_meta) in enumerate(
                                    zip(crops, detection_metadata)
                                ):
...
def process_video_with_enhanced_detection(
    video_path: pathlib.Path, frame_interval: int = None, object_detection_config=None
) -> Tuple[List[FrameInfo], str, dict]:
    """
    ...
    try:
        ...
        logger.info(
            f"Object detection configuration: enabled={enable_object_detection}, "
