...
| `MULTIMODAL_DATAPREP_DEVICE` | Optional | `CPU` | Processing device for embeddings, and object detection (`CPU` or `GPU`). |
...
- **GPU acceleration inactive:** Confirm `/dev/dri/*` is mapped into the container, `MULTIMODAL_DATAPREP_DEVICE=GPU`, and `SDK_USE_OPENVINO=true`.