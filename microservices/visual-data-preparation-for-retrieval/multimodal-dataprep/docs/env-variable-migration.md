...
| Env var | Default | Purpose |
|---|---|---|
| `MM_DATAPREP_INGEST_DATA_ROOT` | `/tmp/dataprep/ingest` | Container root for `POST /videos/ingest-dir`; requested paths are constrained to this root (no traversal). |
...
> Compose-only helper: `MM_DATAPREP_INGEST_DATA_ROOT_HOST` (host path bind-mounted
> to `MM_DATAPREP_INGEST_DATA_ROOT`) — used by Docker Compose, not read by the app.
